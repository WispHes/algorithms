...
Поменяем их местами (в большинстве языков программирования используется функция swap()) и продвинем указатели на следующие элементы. 
...
def quicksort(items):
    def __partition(items, first, last):
        pivot = items[first]
        left, right = first, last
        while left < right:
            if items[right] > pivot:
                right -= 1
            elif items[left] < pivot:
                left += 1
            items[left], items[right] = items[right], items[left]
        return right

    def __quicksort(items, left, right):
        if (right - left) >= 1:
            pivot = __partition(items, left, right)
            __quicksort(items, left, pivot)
            __quicksort(items, pivot+1, right)

    __quicksort(items, 0, len(items)-1)


if __name__ == '__main__':
    players = [
        (-int(score), int(penalty), login) for _ in range(int(input()))
        for login, score, penalty in [input().split()]
    ]
    quicksort(players)
    print(*(login for _, _, login in players), sep="\n")
